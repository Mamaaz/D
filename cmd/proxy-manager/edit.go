...
func runEdit(args []string) {
	...
	default:
		fmt.Fprintf(os.Stderr, "暂未支持编辑该协议: %s (MVP 阶段只覆盖 Reality)\n", protocol)
		fmt.Fprintln(os.Stderr, "其他协议如需改密码/端口，请用 install 重新装一次。")
		os.Exit(2)
	}
}
...
func pickInstalledProtocol(nodes []store.Node) string {
	...
	if len(opts) == 0 {
		fmt.Fprintln(os.Stderr, "已安装的协议都还没有 edit 支持（MVP 阶段只覆盖 Reality）")
		return ""
	}
	if len(opts) == 1 {
		fmt.Printf("选中: %s\n", opts[0].desc)
